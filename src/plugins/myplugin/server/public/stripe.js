...
function SS_ProductCheckout() {
  ...
  const getProductApi = baseUrl + "/myplugin/getProduct/" + productId;
  const checkoutSessionUrl = baseUrl + "/myplugin/createCheckoutSession/";
...
function SS_GetProductPaymentDetails(checkoutSessionId) {
  ...
  const retrieveCheckoutSessionUrl =
    baseUrl + "/myplugin/retrieveCheckoutSession/" + checkoutSessionId;
    ...
    .then((response) => {
      if (response.payment_status === "paid") {
        if (
          ...
          const stripePaymentUrl = baseUrl + "/myplugin/stripePayment";
